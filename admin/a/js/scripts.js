...
function _get_active_link(divid) {
  $(
    "#side-dashboard, #side-staff, #side-event, #side-gallery, #side-blog, #side-faq, #side-test"
...
function _get_active_modal_link(menu_id) {
  $(
    "#page_content, #video_page, #audio_page, #material_page, #picture_page"
  ).removeClass("active-li");
...
function _check_page_content(menu_id, page, publish_id) {
  ...
  const dataString =
    "action=" + action + "&page=" + page + "&publish_id=" + publish_id;
  $.ajax({
    type: "POST",
    url: admin_local_portal_url,
    data: dataString,
    ...
    $.ajax({
      ...
      success: function (info) {
        ...
        if (success == true) {
          ...
          $(
            "#login_fullname,#header_fullname,#pro_header_fullname,#login_user_fullname"
          ).html(fullname);
...
function _getSelectGalleryType(select_id) {
  $.ajax({
    type: "POST",
    url: endPoint + "/setups/gallery-type",
    dataType: "json",
    cache: false,
    headers: {
      apiKey: apiKey,
    ...
    success: function (info) {
      ...
      if (success == true) {
        for (let i = 0; i < fetch.length; i++) {
          const gallery_type_id = fetch[i].gallery_type_id;
          const gallery_type_name = fetch[i].gallery_type_name;
          $("#" + select_id).append(
            '<option value="' +
              gallery_type_id +
              '">' +
...
function _fetchAllStaff() {
  ...
  if (search_keywords.length > 2 || search_keywords === "") {
    const dataString =
      "status_id=" + status_id + "&search_keywords=" + search_keywords;
    ...
    try {
      $.ajax({
          ...
          if (success === true) {
            if (!fetch) {
              text +=
                ...
                "'staff_reg'" +
                ')"><i class="bi-person-plus"></i> ADD A NEW ADMIN</button>' +
                "</div>";
            } else {
              for (let i = 0; i < fetch.length; i++) {
                ...
                text +=
                  '<div class="user-div animated fadeIn" title="CLICK TO VIEW ' +
                  fullname +
                  " PROFILE\" onclick=\"_get_form_with_id('staff_profile', '" +
                  staff_id +
                  "')\")>" +
        ...
        error: function (textStatus, errorThrown) {
          console.error("AJAX Error: ", textStatus, errorThrown);
          $("#fetchAllStaff").html(
            '<div class="false-notification-div"><p>An error occurred while fetching data. Please try again.</p></div>'
          );
        },
      });
    } catch (error) {
      console.error("Error: ", error);
      $("#fetchAllStaff").html(
        '<div class="false-notification-div"><p>An unexpected error occurred. Please try again.</p></div>'
      );
    }
  }
}
...
function _addStaff() {
  try {
    ...
    $(
      "#fullname, #email, #phone, #address, #position, #role_id, #status_id"
    ).removeClass("issue");
    ...
    if (confirm("Confirm!!\n\n Are you sure to PERFORM THIS ACTION?")) {
      ...
      $("#submit_btn").html(
        '<img src="' +
          website_url +
          '/all-images/images/loading.gif" width="12px" alt="Loading"/> Processing'
      );
...
function _updateLoginStaff() {
  try {
    ...
    $(
      "#updt_fullname, #updt_email, #updt_mobile, #updt_address, #updt_position, #updt_role_id, #updt_status_id"
    ).removeClass("issue");
          ...
          if (success === true) {
            if (!fetch) {
              text +=
                ...
                '<button class="btn" onclick="_get_form(' +
                "'event_reg'" +
                ')"><i class="bi-person-plus"></i> ADD A NEW EVENT</button>' +
                "</div>";
            } else {
              for (let i = 0; i < fetch.length; i++) {
                ...
                text +=
                  ...
                  '<div class="btn-div">' +
                  '<button class="btn active-btn" onclick="_get_pages_form_with_id(' +
                  "'update_event'" +
                  "," +
                  "'" +
                  page_category_id +
                  ...
                  "'" +
                  ')">EDIT</button>' +
                  '<button class="btn" onclick="_edit_page(' +
                  "'" +
                  page_category_id +
                  "'," +
                  "'" +
                  ...
                  '"></div>' +
                  '<div class="text-div">' +
                  '<div class="top-text"><span><i class="bi-calendar-check"></i> </span><em>' +
                  event_start_time +
                  " - " +
                  event_end_time +
                  "</em> </div>" +
                  ...
                  "...</h2>" +
                  '<div class="text-in">' +
                  '<div class="text">UPDATED ON: <span>' +
                  formattedDate +
                  "</span> | <span>" +
                  page_view +
                  "</span> VIEWS </div>" +
...
function _fetchEachEvent(categoryIdToFind, publishId) {
  ...
  const event = fetchEvent.find(
    (s) => s.page_category_id === categoryIdToFind && s.publish_id === publishId
  );
...
function _addEvent(page_category_id) {
  try {
    ...
    const raw_start_time = $("#event_start_time").val();
    const raw_end_time = $("#event_end_time").val();

    const event_start_time = convertToAmPm(raw_start_time);
    const event_end_time = convertToAmPm(raw_end_time);
    ...
    $(
      "#reg_title, #event_date, #event_start_time, #event_end_time, #event_location, #reg_thumbnail, #status_id"
    ).removeClass("issue");
...
function _updateEvent(page_category_id, publish_id) {
  try {
    ...
    $(
      "#reg_title, #event_date, #event_start_time, #event_end_time, #event_location, #reg_thumbnail, #uptd_status_id"
    ).removeClass("issue");
          ...
          if (success === true) {
            if (!fetch) {
              text +=
                ...
                '<button class="btn" onclick="_get_form(' +
                "'gallery_reg'" +
                ')"><i class="bi-person-plus"></i> ADD A NEW GALLERY</button>' +
                "</div>";
            } else {
              for (let i = 0; i < fetch.length; i++) {
                const page_category_id = fetch[i].page_category_id;
                ...
                const reg_title = fetch[i].reg_title.substr(0, 50);
                const gallery_type_id = fetch[i].gallery_type_id;
                const class_name = fetch[i].class_name;
                const updated_time = fetch[i].updated_time;
                const status_name = fetch[i].status_name;
                const reg_pix = fetch[i].reg_pix;
                const documentStoragePath = fetch[i].documentStoragePath;
				...
				<div class="grid-div" data-aos="fade-in" data-aos-duration="1500">
					...
					<button 
						...
						onclick="_get_pages_form_with_id('update_gallery','${page_category_id}','${publish_id}')"
					>
						EDIT
					</button>
					<button 
						class="btn" 
						onclick="_edit_page('${page_category_id}','${publish_id}')"
					>
						EDIT PAGE DETAILS
					</button>
					</div>
					...
					${
            gallery_type_id === "CG"
              ? `<div class="top-text"><span>${class_name}</span></div>`
              : ""
          }
					<h2>${reg_title}</h2>
					<div class="text-in">
...
function _fetchEachGallery(categoryIdToFind, publishId) {
  ...
  const gallery = fetchGallery.find(
    (s) => s.page_category_id === categoryIdToFind && s.publish_id === publishId
  );
...
function _addGallery(page_category_id) {
  try {
    ...
    $(
      "#gallery_type_id, #reg_title, #class_name, #reg_thumbnail, #status_id"
    ).removeClass("issue");
    ...
    $.ajax({
      ...
      success: function (info) {
        ...
        if (success == true) {
          ...
          $("#seo_flyer_preview_pix").attr(
            "src",
            documentStoragePath + "/" + seo_flyer
          );
...
function _addPageContent(publish_id) {
  ...
  try {
    ...
    $(
      "#page_url, #page_title, #seo_keywords, #seo_description, #page_content_text, #seo_flyer"
    ).removeClass("issue");
    ...
    $(
      "#page_url, #page_title, #seo_keywords, #seo_description, #page_content, #seo_flyer"
    ).removeClass("issue");
        ...
        if (success === true) {
          if (!fetch) {
            ...
            for (let i = 0; i < fetch.length; i++) {
              ...
              text +=
                '<div class="video-div picture-div" id="pictures-' +
                sn +
                '">' +
                '<div class="icon-div" onclick="deletePagePictures(' +
                "'" +
                publish_id +
                "'," +
                "'" +
    ...
    $.ajax({
      ...
      success: function (info) {
        ...
        if (success == true) {
          ...
          const total_active_testimony_count =
            data.total_active_testimony_count;
          ...
          if (success === true) {
            if (!fetch) {
              text +=
                ...
                '<button class="btn" onclick="_get_form(' +
                "'blog_reg'" +
                ')"><i class="bi-person-plus"></i> ADD A NEW BLOG</button>' +
                "</div>";
            } else {
              for (let i = 0; i < fetch.length; i++) {
                const page_category_id = fetch[i].page_category_id;
                ...
                text +=
                  ...
                  '"></div>' +
                  '<div class="text-div">' +
                  '<div class="top-text">Category: <span> ' +
                  cat_name +
                  " </span></div>" +
                  "<h2>" +
                  reg_title +
...
function _updateBlog(page_category_id, publish_id) {
  try {
    ...
    $("#reg_title, #cat_id, #reg_thumbnail, #uptd_status_id").removeClass(
      "issue"
    );
          ...
          if (success === true) {
            if (!fetch) {
              text +=
                ...
                '<button class="btn" onclick="_get_form(' +
                "'faq_reg'" +
                ')"><i class="bi-person-plus"></i> ADD A NEW FAQ</button>' +
                "</div>";
            } else {
              for (let i = 0; i < fetch.length; i++) {
                no++;
                ...
                text +=
                  ...
                  no +
                  "</div>" +
                  '<button class="btn" onClick="_get_pages_form_with_id(' +
                  "'update_faq'" +
                  "," +
                  "'" +
                  page_category_id +
                  ...
                  publish_id +
                  "'" +
                  ')"><i class="bi-pencil-square"></i> <span>' +
                  faq_question +
                  "</span></button>" +
                  "</div>" +
...
function _updateFaq(page_category_id, publish_id) {
  ...
  try {
    ...
    $("#cat_id, #faq_question, #faq_answer, #updt_status_id").removeClass(
      "issue"
    );
            ...
            if (!fetch) {
              ...
              for (let i = 0; i < fetch.length; i++) {
                ...
                text +=
                  ...
                  '<div class="details">' +
                  '<div class="pix"><img src="' +
                  website_url +
                  '/all-images/images/quote.png" alt="Quote"/></div>' +
                  '<div class="text">' +
                  "<h3>" +
                  fullname +
                  "</h3>" +
                  ...
                  "</div>" +
                  "</div>" +
                  '<button class="btn" onClick="_get_form_with_id(' +
                  "'update_testimony'" +
                  "," +
                  "'" +
                  testimony_id +
...
function _updateTestimony(testimony_id) {
  try {
    ...
    $(
      "#fullname, #email, #phone, #updt_status_id, #relationship_type_id, #testimony"
    ).removeClass("issue");
...
function _updateSettings() {
  try {
    ...
    $(
      "#sender_name, #smtp_host, #smtp_username, #smtp_password, #smtp_port, #support_email"
    ).removeClass("issue");
    ...
    if (confirm("Confirm!!\n\n Are you sure to PERFORM THIS ACTION?")) {
      ...
      $("#update_btn").html(
        '<img src="' +
          website_url +
          '/all-images/images/loading.gif" width="12px" alt="Loading"/> Updating'
      );
      document.getElementById("update_btn").disabled = true;
...
function _changePassword() {
  ...
  if (!old_password) {
    ...
    $("#old_password, #new_password, #confirm_password").removeClass(
      "complain"
    );
    ...
    if (confirm("Confirm!!\n\n Are you sure to PERFORM THIS ACTION?")) {
      ...
      $("#submit_btn").html(
        '<img src="' +
          website_url +
          '/all-images/images/loading.gif" width="12px" alt="Loading"/> Resetting'
      );
      document.getElementById("submit_btn").disabled = true;
      ////////////////////////////////////////////////
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          if (success == true) {
            _actionAlert(message, true);
            _get_form("password_reset_successful");
          } else {
            _actionAlert(message, false);
          }
          $("#submit_btn").html(btn_text);
      ...
      $("#new_password,#confirm_password").addClass("complain");
      _actionAlert(
        "Password Not Accepted, Please follow the instructon",
        false
      );
    }
  }
        ...
        if (success === true) {
          if (!fetch) {
            ...
            for (let i = 0; i < fetch.length; i++) {
              ...
              const user_name = capitalizeFirstLetterOfEachWord(
                fetch[i].user_name
              );
              ...
              text +=
                '<div class="system-alert" id="' +
                alert_id +
                '" onclick="_get_form_with_id(' +
                "'alert-read'" +
                "," +
                "'" +
                alert_id +
                "'" +
                ')">' +
                '<div class="alert-name"><i class="bi-person"></i> ' +
                user_name +
                '<span id="' +
                alert_id +
                'viewed"><i class="bi-check"></i></span></div>' +
                '<div class="alert-text">Success Alert: ' +
                alert_detail +
                "...</div>" +
                '<div class="alert-time"><i class="bi-clock"></i> <span>' +
                created_time +
                "</span></div>" +
                "</div>";
            }
      ...
      if (success == true) {
        ...
        if (unread_alert >= 100) {
          ...
          $(".bell_notification").html(
            '<i class="bi-bell"></i><div>' + unread_alert + "</div>"
          );
        }
      } else {
        const response = info.response;
...
function _getCustomReport(datefrom, dateto, view_report) {
  ...
  if (datefrom == "" || dateto == "") {
    ...
    $("#warning-div")
      .html(
        '<div><i class="bi-exclamation-circle"></i></div>DATE ERROR<br /> <span>Select date to continue</span>'
      )
      .fadeIn(500)
      .delay(3000)
      .fadeOut(100);
...
function _fetchAllSystemAlert(dataString) {
  $("#fetchAllSystemAlert")
    .html(
      '<div class="ajax-loader alert-ajax-loader"><img src="' +
        website_url +
        '/all-images/images/spinner.gif" alt="Loading"/></div>'
    )
    .fadeIn("fast");
        ...
        if (success === true) {
          if (!fetch) {
            ...
            for (let i = 0; i < fetch.length; i++) {
              ...
              if (seen_status == 0) {
                text +=
                  '<div class="system-alert main-system-alert" id="' +
                  alert_id +
                  '" onclick="_get_form_with_id(' +
                  "'alert-read'" +
                  "," +
                  ...
                  '<span id="' +
                  alert_id +
                  'viewed"><i class="bi-check2"></i></span></div>' +
                  '<div class="alert-text">Success Alert: ' +
                  alert_detail +
                  "...</div>" +
                  '<div class="alert-time"><i class="bi-clock"></i> <span>' +
                ...
                text +=
                  '<div class="system-alert main-system-alert alert-seen" id="' +
                  alert_id +
                  '" onclick="_get_form_with_id(' +
                  "'alert-read'" +
                  "," +
                  ...
                  '<span id="' +
                  alert_id +
                  'viewed"><i class="bi-check2-all"></i></span></div>' +
                  '<div class="alert-text">Success Alert: ' +
                  alert_detail +
                  "...</div>" +
                  '<div class="alert-time"><i class="bi-clock"></i> <span>' +
...
function _fetchNextAlerts() {
  ...
  if (view_to < all_record_count) {
    const dataString =
      "search_keywords=" + search_keywords + "&view_from=" + view_from;
    _fetchAllSystemAlert(dataString);
  }
}
